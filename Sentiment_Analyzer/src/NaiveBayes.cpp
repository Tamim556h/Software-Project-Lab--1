...
void NaiveBayes::trainFromDocuments(const std::vector<std::vector<std::string>> &docs, 
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (!classExists(lab)) classes.push_back(lab);
        if (classDocCount.find(lab) == classDocCount.end()) classDocCount[lab] = 0;
        // ensure maps exist
        if (totalWordsInClass.find(lab) == totalWordsInClass.end()) totalWordsInClass[lab] = 0;
        if (wordCountPerClass.find(lab) == wordCountPerClass.end()) wordCountPerClass[lab] = std::map<std::string,int>();
    }
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (size_t t = 0; t < tokens.size(); ++t) {
            ...
            if (wordCountPerClass[lab].find(w) == wordCountPerClass[lab].end()) wordCountPerClass[lab][w] = 1;
            else wordCountPerClass[lab][w] += 1;
    ...
    for (size_t i = 0; i < classes.size(); ++i) {
        ...
        for (int v = 0; v < vocabSize; ++v) {
            ...
            int cnt = 0;
            if (wordCountPerClass[c].find(w) != wordCountPerClass[c].end()) cnt = wordCountPerClass[c][w];
...
std::string NaiveBayes::predict(const std::vector<std::string> &tokens) {
    ...
    for (size_t i = 0; i < classes.size(); ++i) {
        ...
        // prior
        double pC = 1e-12;
        if (priorProb.find(c) != priorProb.end()) pC = priorProb[c];
        if (pC <= 0.0) score = -INFINITY;
        else score = std::log(pC);

        // add log-likelihoods
        for (size_t t = 0; t < tokens.size(); ++t) {
            const std::string &w = tokens[t];
            double pwc = 0.0;
            if (condProb[c].find(w) != condProb[c].end()) pwc = condProb[c][w];
            else {
                ...
                if (denom <= 0.0) pwc = 1.0 / (double)(vocabSize + 1);
                else pwc = 1.0 / denom;
            }
            if (pwc > 0.0) score += std::log(pwc);
            else score += -1e9;
        }
